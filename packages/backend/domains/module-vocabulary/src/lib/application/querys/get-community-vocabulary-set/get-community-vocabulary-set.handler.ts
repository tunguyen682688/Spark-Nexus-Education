...
import {
  ...
  GetCommunityVocabularySetQuery,
} from './get-community-vocabulary-set.query';
...
@QueryHandler(GetCommunityVocabularySetQuery)
export class GetCommunityVocabularySetQueryHandler
  implements
    IQueryHandler<
      GetCommunityVocabularySetQuery,
      CommunityVocabularySetListResult
    >
{
  ...
  async execute(
    query: GetCommunityVocabularySetQuery
  ): Promise<CommunityVocabularySetListResult> {
    ...
    if (!normalizedParams.sort || normalizedParams.sort.length === 0) {
      normalizedParams.sort = [
        { field: 'favoriteCount', direction: SortDirection.DESC },
        { field: 'entryCount', direction: SortDirection.DESC },
        { field: 'createdAt', direction: SortDirection.DESC },
      ];
    }

    // Repository will enforce community filters (public, active, not deleted)
    const result =
      await this.vocabularySetRepository.findCommunityVocabularySets(
        normalizedParams
      );
    ...
    const meta: ResponseMeta = {
      version: '1.0.0',
      message: 'Community vocabulary sets retrieved successfully',
      pagination: {
        page: result.page,
        limit: result.limit,
        total: result.total,
