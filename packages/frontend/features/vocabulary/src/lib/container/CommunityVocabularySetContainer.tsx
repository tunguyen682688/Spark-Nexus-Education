...
import {
  Loader2, BookOpen, Plus, Heart, Search,
  ChevronDown, Star, Award, Sparkles, Users, Library, TrendingUp
} from 'lucide-react';
import {
  useToast,
  Card,
...
import { useCommunityVocabularySets, useToggleFavorite } from '../hooks/use-vocabulary-sets';
...
function levelInfo(entryCount: number) {
  if (entryCount > 40) return { label: 'C1 · ADVANCED',          cls: 'bg-red-500/10 text-red-600 dark:text-red-400 border-red-500/20' };
  if (entryCount > 25) return { label: 'B2 · INTERMEDIATE',      cls: 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20' };
  return                       { label: 'A2 · ELEMENTARY',        cls: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20' };
}
...
export default function CommunityVocabularySetContainer() {
  ...
  const [accumulatedSets, setAccumulatedSets]   = useState<CommunityVocabularySet[]>([]);
  ...
  // Fetch
  const { data: setsData, isLoading, error } = useCommunityVocabularySets(queryParams);
  ...
  const hasMore        = paginationMeta ? currentPage < paginationMeta.totalPages : false;

  // ── Derive real top-contributors from the data we already have ──────────────
  // No mock fallback — show skeleton / empty state if no data yet.
  const topContributors = useMemo(() => {
    const map = new Map<string, { name: string; initial: string; sets: number }>();
  ...
  const handleToggleFavorite = async (setId: string, isFavorited: boolean) => {
    try {
      await toggleFavoriteMutation.mutateAsync({ setId, isFavorited: !isFavorited });
      setAccumulatedSets((prev) =>
        prev.map((s) =>
          s.id === setId
            ? { ...s, isFavorited: !isFavorited, favoriteCount: s.favoriteCount + (!isFavorited ? 1 : -1) }
            : s
        )
      );
      toast({
      ...
      toast({ title: 'Lỗi', description: 'Không thể cập nhật yêu thích. Thử lại sau.', variant: 'destructive' });
    }
  };
  ...
  // ── Skeleton helpers ─────────────────────────────────────────────────────────
  const skeletonCard  = (key: number) => <div key={key} className="h-64 rounded-2xl bg-slate-100 dark:bg-slate-800 animate-pulse" />;
  const skeletonRow   = (key: number) => <div key={key} className="h-20 rounded-2xl bg-slate-100 dark:bg-slate-800 animate-pulse" />;

  // ── JSX ─────────────────────────────────────────────────────────────────────
  return (
    <div className="w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 select-none">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 p-8 sm:p-10">
          ...
          <div className="flex-1 flex flex-col gap-4 text-left">
            ...
            <p className="text-sm sm:text-base text-slate-600 dark:text-slate-400 font-medium max-w-lg">
              Khám phá hàng ngàn bộ từ vựng chất lượng cao được tạo bởi các chuyên gia và học viên trên toàn thế giới.
            </p>
                      ...
                      >
                        ...
                        <div className="relative h-36 overflow-hidden bg-slate-800 shrink-0">
                          ...
                          <span className="absolute bottom-2.5 left-2.5 text-[10px] font-bold text-white/90 bg-black/30 backdrop-blur-sm px-2 py-0.5 rounded">
                            {set.language?.toUpperCase() || 'EN'} · {set.entryCount} từ
                          </span>
                        </div>
                          ...
                          <div className="flex items-center justify-between mt-auto pt-2 border-t border-slate-50 dark:border-slate-800">
                            <div className="flex items-center gap-0.5 text-amber-500">
                              ...
                              <span className="text-xs font-bold">
                                {(4.5 + (parseInt(set.id.substring(0, 2), 16) % 6) * 0.1).toFixed(1)}
                              </span>
                              <span className="text-[10px] text-slate-400 dark:text-slate-500 ml-1">({set.favoriteCount})</span>
                            </div>
                            <button
                              onClick={(e) => { e.stopPropagation(); handleToggleFavorite(set.id, isFav); }}
            ...
            <div className="flex items-center justify-between gap-4 flex-wrap">
              ...
              <div className="flex gap-1 bg-slate-100 dark:bg-slate-800 p-0.5 rounded-xl border border-slate-200 dark:border-slate-700">
                {(['featured', 'recent', 'popular'] as const).map((tab) => (
                  ...
                  >
                    {tab === 'featured' ? 'Phổ biến' : tab === 'recent' ? 'Mới nhất' : 'Yêu thích'}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3">
              {isLoading && accumulatedSets.length === 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
                  ...
                  <h3 className="font-bold text-slate-700 dark:text-slate-300">Không tìm thấy học liệu</h3>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 flex flex-col gap-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded border ${lvlCls}`}>
                            {lvl}
                          </span>
                        ...
                        <div className="flex items-center gap-3 text-[11px] text-slate-400 dark:text-slate-500 font-medium">
                          {set.creator?.name && (
                            <span className="truncate max-w-[140px]">
                              By <span className="text-slate-600 dark:text-slate-300 font-semibold">{set.creator.name}</span>
                            </span>
                          )}
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <Button
                          variant="default"
                          size="sm"
                          onClick={(e) => { e.stopPropagation(); handleViewSet(set.id); }}
            ...
            {hasMore && (
              <div className="flex justify-center pt-2">
                ...
                >
                  {isLoading
                    ? <Loader2 className="h-4 w-4 animate-spin" />
                    : <><span>Tải thêm kết quả</span><ChevronDown className="h-4 w-4" /></>}
                </Button>
              </div>
            )}
          </section>
          ...
          <Card className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
            ...
            <CardContent className="p-3 space-y-1">
              {isLoading && topContributors.length === 0 ? (
                [0, 1, 2, 3].map((k) => (
                  <div key={k} className="h-10 rounded-xl bg-slate-100 dark:bg-slate-800 animate-pulse" />
                ))
                ...
                topContributors.map((contributor, index) => (
                  ...
                  >
                    {/* Rank badge */}
                    <div className={`flex items-center justify-center w-6 h-6 rounded-full text-[11px] font-black shrink-0 ${RANK_COLOURS[index] ?? RANK_COLOURS[3]}`}>
                      {index + 1}
                    </div>
                    {/* Avatar initial */}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate">{contributor.name}</p>
                      <p className="text-[10px] text-slate-400 dark:text-slate-500">{contributor.sets} học liệu</p>
                    </div>
                    {/* Score badge */}
          ...
          <Card className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
            ...
            <CardContent className="p-4 space-y-3">
              ...
              <div className="flex items-center gap-3 p-3 bg-blue-50 dark:bg-blue-950/20 rounded-xl border border-blue-100 dark:border-blue-900/40">
                ...
                <div>
                  <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Tài liệu</p>
              ...
              <div className="flex items-center gap-3 p-3 bg-emerald-50 dark:bg-emerald-950/20 rounded-xl border border-emerald-100 dark:border-emerald-900/40">
                ...
                <div>
                  <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Người đóng góp</p>
                  <p className="text-lg font-black text-emerald-600 dark:text-emerald-400 leading-none mt-0.5">
                    {topContributors.length > 0 ? `${topContributors.length}+` : '—'}
                  </p>
                </div>
              </div>
