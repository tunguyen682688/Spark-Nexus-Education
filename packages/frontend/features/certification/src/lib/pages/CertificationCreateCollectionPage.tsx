...
export const CertificationCreateCollectionPage: React.FC = () => {
  const navigate = useNavigate();
  const text = CERTIFICATION_UI_TEXT.placeholderPages.createCollection;
  ...
  return (
    <div className="w-full space-y-6 pb-20">
      <div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground">
        ...
        <span>&gt;</span>
        <span className="text-foreground font-bold">{bc.createCollection}</span>
      </div>

      <div className="flex flex-col items-center justify-center py-20 space-y-4">
        <div className="w-16 h-16 rounded-2xl bg-amber-100 dark:bg-amber-950/60 flex items-center justify-center">
