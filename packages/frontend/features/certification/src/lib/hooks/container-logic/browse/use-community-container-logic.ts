...
const COMMUNITY_HIGHLIGHT_STATS: CommunityHighlightStat[] = [
  { label: CERTIFICATION_UI_TEXT.community.stats.collections, displayValue: '125K+', description: CERTIFICATION_UI_TEXT.community.stats.collectionsDesc, Icon: Users, color: 'text-emerald-500 bg-emerald-50 dark:bg-emerald-950/20' },
  { label: CERTIFICATION_UI_TEXT.community.stats.rating, displayValue: '4.8', description: CERTIFICATION_UI_TEXT.community.stats.ratingDesc, Icon: Star, color: 'text-amber-500 bg-amber-50 dark:bg-amber-950/20' },
  { label: CERTIFICATION_UI_TEXT.community.stats.downloads, displayValue: '2.6M+', description: CERTIFICATION_UI_TEXT.community.stats.downloadsDesc, Icon: Download, color: 'text-blue-500 bg-blue-50 dark:bg-blue-950/20' },
  { label: CERTIFICATION_UI_TEXT.community.stats.feedback, displayValue: '98%', description: CERTIFICATION_UI_TEXT.community.stats.feedbackDesc, Icon: Heart, color: 'text-purple-500 bg-purple-50 dark:bg-purple-950/20' },
];
...
export function useCommunityContainerLogic(onStartExam?: (examId: string) => void) {
  ...
  const handleViewCollectionDetail = (collectionId: string) => {
    if (onStartExam) onStartExam(collectionId);
    else navigate(`/certification/collections/${collectionId}`);
  };
  ...
  return {
    ...
    communityHighlightStats: COMMUNITY_HIGHLIGHT_STATS,
