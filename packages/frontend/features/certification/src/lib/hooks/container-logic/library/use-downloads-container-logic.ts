...
export function useDownloadsContainerLogic() {
  ...
  useEffect(() => {
    if (apiData) {
      const response = apiData as unknown as DownloadsResponse;
      const fetched: DownloadFileItem[] = (response.items ?? []).map((item) => ({
        id: item.id,
        name: item.title,
        subtitle: '',
        fileFormat: 'PDF' as const,
        fileFormatBadgeClass: '',
        fileIconBgClass: '',
        downloadedOn: item.downloadedAt,
        size: item.fileSize,
        expiresOn: '',
