    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1 font-medium">{text.SUBTITLE}</p>
        </div>
      ...
      >
        {trendingItems.map((material) => (
          <div
            ...
            className="min-w-[280px] sm:min-w-[340px] max-w-[340px] snap-start group relative flex flex-col bg-card border border-border hover:border-primary/40 rounded-2xl overflow-hidden cursor-pointer shadow-lg hover:shadow-2xl hover:shadow-primary/5 transition-all duration-300"
          >
            {/* Thumbnail */}
            <div className="relative aspect-[16/10] bg-muted overflow-hidden">
              {material.thumbnailUrl ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-primary/10 to-muted">
                  <Headset className="w-12 h-12 text-primary/20" />
                </div>
              )}
            ...
            <div className="flex-1 flex flex-col p-4 space-y-2 justify-between">
              ...
              <div className="flex items-center justify-between text-xs text-muted-foreground border-t border-border/60 pt-3 mt-2">
                ...
                <span className="flex items-center gap-1 text-[10px] font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-full border border-primary/20">
                  <Sparkle className="w-3 h-3 text-primary fill-current" />
