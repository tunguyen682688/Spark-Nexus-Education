...
export const HubLeaderboard: React.FC<HubLeaderboardProps> = ({
  ...
  return (
    <Card className="bg-slate-900/40 border border-slate-800 rounded-3xl p-6 backdrop-blur-md space-y-5">
      <div>
        <h3 className="text-xs font-black uppercase tracking-wider text-slate-400 flex items-center gap-2">
          <Trophy className="w-4 h-4 text-orange-400" />
          Bảng Xếp Hạng Luyện Nghe Tuần
        </h3>
        <p className="text-[10px] text-slate-500 mt-0.5">Top 5 học viên tích cực nhất</p>
      </div>
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          leaderboardData.map((entry, index) => {
            ...
            let borderClass = "border-slate-850 hover:border-slate-700";
            ...
            return (
              <div
                key={entry.userId}
                className={`flex items-center justify-between p-2.5 rounded-xl bg-slate-950/30 border transition-colors ${borderClass}`}
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-[11px] font-extrabold text-slate-200 leading-tight">
                      {entry.userName}
                    </h4>
                ...
                <span className={`text-[11px] font-black ${isTop1 ? 'text-yellow-400' : isTop2 ? 'text-slate-350' : isTop3 ? 'text-amber-500' : 'text-slate-400'}`}>
                  {minutes} phút
                </span>
              </div>
            );
