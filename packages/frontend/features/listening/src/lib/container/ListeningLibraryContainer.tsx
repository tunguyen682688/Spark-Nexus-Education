...
import { Button, Tooltip, TooltipTrigger, TooltipContent, TooltipProvider } from '@spark-nest-ed/frontend-shared-components';
              ...
              <div className="space-y-6">
                ...
                <div className="flex border-b border-border/80 pb-0.5 gap-2">
                  ...
                  <Button
                    ...
                    className={`flex items-center gap-2 px-5 py-3 text-xs font-extrabold border-b-2 transition-all rounded-none bg-transparent hover:bg-transparent ${
                  ...
                  <>
                      ...
                      inProgressItems.length === 0 ? (
                        <div className="text-center py-20 bg-card border border-border border-dashed rounded-3xl text-muted-foreground space-y-3">
                          ...
                          <Button
                            variant="link"
                            onClick={() => navigate(LISTENING_ROUTES.HUB)}
                            className="text-xs text-primary hover:underline font-extrabold flex items-center justify-center gap-1.5 mx-auto h-auto p-0"
                          >
                    ...
                    {activeTab === 'completed' && (
                      completedItems.length === 0 ? (
                        <div className="text-center py-16 bg-card border border-border border-dashed rounded-3xl text-muted-foreground space-y-2">
                          <Trophy className="w-12 h-12 text-slate-700 mx-auto" />
                          <p className="text-xs font-bold">{text.COMPLETED_EMPTY.TITLE}</p>
                          <p className="text-[10px] text-slate-500 max-w-sm mx-auto leading-relaxed">{text.COMPLETED_EMPTY.DESC}</p>
                        </div>
                      ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    {activeTab === 'bookmarks' && (
                      bookmarkedItems.length === 0 ? (
                        <div className="text-center py-16 bg-card border border-border border-dashed rounded-3xl text-muted-foreground space-y-2">
                          ...
                          <p className="text-[10px] text-slate-500 max-w-sm mx-auto leading-relaxed">{text.BOOKMARKS_EMPTY.DESC}</p>
                        </div>
                      ) : (
                        <div className="space-y-4">
                          {bookmarkedItems.map((item) => (
                            <div
                              key={item.id}
                              className="group flex items-center justify-between p-4 bg-card border border-border rounded-2xl hover:border-primary/20 hover:shadow-lg transition-all duration-300 gap-4"
                            >
                              <div
                                onClick={() => navigate(LISTENING_ROUTES.STUDY(item.id))}
                              ...
                              >
                                <div className="w-12 h-12 rounded-xl overflow-hidden bg-muted border border-border shrink-0 flex items-center justify-center">
                                  {item.thumbnailUrl ? (
                                    ...
                                    <Bookmark className="w-5 h-5 text-primary/45" />
                                  )}
                                </div>
                                <div className="min-w-0">
                                  ...
                                  <div className="flex items-center gap-1.5 text-[9px] text-muted-foreground font-semibold mt-1">
                                    <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-1.5 py-0.2 rounded uppercase">
                                      {item.difficulty}
                                    </span>
                                    <span>•</span>
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    className="p-2 h-9 w-9 rounded-xl bg-muted border border-border text-muted-foreground hover:text-red-400 hover:border-red-500/30 active:scale-95 transition-all shrink-0"
                                  >
                                    <Trash2 className="w-4 h-4" />
                                  </Button>
                                </TooltipTrigger>
                                <TooltipContent>
                                  <p className="text-[10px] font-bold">{text.BOOKMARK_ITEM.REMOVE_TITLE}</p>
                                </TooltipContent>
                              </Tooltip>
                            </div>
                          ))}
            ...
            <div className="space-y-6">
              ...
              <LibraryStatsSummaryCard
                ...
                averageProgress={
                  allItems.length === 0
                    ? 0
                    : Math.round(allItems.reduce((acc, curr) => acc + (curr.userProgress?.progress || 0), 0) / allItems.length)
                }
              />

              {/* WIDGET 2: CEFR DIFFICULTY LEVELS BREAKDOWN */}
              <LibraryCEFRBreakdownCard getCountByLevel={getCountByLevel} />

              {/* WIDGET 3: HISTORICAL ACTIVITY WEEKLY CHART */}
              <LibraryWeeklyChart weeklyData={weeklyData} />

              {/* WIDGET 4: USER ACHIEVEMENTS GRID */}
