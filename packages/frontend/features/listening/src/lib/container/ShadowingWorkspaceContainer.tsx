import { useShadowingWorkspace } from '../hooks';
import {
  ArrowLeft,
  RefreshCw,
  Info,
...
import ShadowingSidebar from '../components/ShadowingSidebar';
import ShadowingExerciseCard from '../components/ShadowingExerciseCard';
import { LISTENING_WORKSPACE_TEXT, LISTENING_ROUTES } from '../constants';

export default function ShadowingWorkspaceContainer() {
  const {
    navigate,
    audioRef,
    userAudioRef,
  ...
  return (
    <div className="min-h-screen bg-slate-955 text-slate-100 flex flex-col">
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden max-w-full mx-auto w-full p-6 gap-6 h-[calc(100vh-80px)]">
        {/* Left column: Subtitle select list */}
        <ShadowingSidebar
          subtitles={subtitles}
        ...
        <div className="flex-1 flex flex-col gap-6 overflow-y-auto pr-1 h-full">
          ...
          {currentSub ? (
            <ShadowingExerciseCard
              ...
              handlePlayUserAudio={handlePlayUserAudio}
              setSelectedSubIndex={setSelectedSubIndex}
