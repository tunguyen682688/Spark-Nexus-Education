    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col h-screen overflow-hidden">
      ...
      <header className="bg-card border-b border-border backdrop-blur-md px-6 py-4 flex items-center justify-between sticky top-0 z-40 shrink-0">
        ...
        <div className="flex items-center gap-4 text-xs font-bold text-muted-foreground">
          <div className="flex items-center gap-1.5 px-3 py-1 bg-secondary border border-border rounded-xl">
          ...
          {submittedCount === questions.length && questions.length > 0 && (
            <div className="p-8 bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20 rounded-2xl text-center space-y-5 backdrop-blur-md shadow-2xl animate-in fade-in slide-in-from-top-4 duration-300">
              ...
              <div className="flex justify-center gap-8 py-2 text-xs font-bold uppercase tracking-wider">
                <div className="bg-secondary/40 border border-border px-5 py-3.5 rounded-xl text-center min-w-[110px]">
                  <span className="text-emerald-400 text-3xl font-black block">{correctCount}</span>
                  <span className="text-muted-foreground text-[10px]">{text.CONGRATS_CORRECT_VAL(correctCount, questions.length)}</span>
                </div>
                <div className="bg-secondary/40 border border-border px-5 py-3.5 rounded-xl text-center min-w-[110px]">
                  <span className="text-primary text-3xl font-black block">
                    {questions.length > 0 ? Math.round((correctCount / questions.length) * 100) : 0}%
                  </span>
                  <span className="text-muted-foreground text-[10px]">{text.CONGRATS_ACCURACY}</span>
                </div>
              </div>
              <button
                onClick={() => navigate(LISTENING_ROUTES.STUDY(material.id))}
